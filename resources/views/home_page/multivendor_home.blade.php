...
<div class="siddhi-home-page">
    ...
    <script type="text/javascript">
        ...
        async function callStore() {
            ...
            vendorNearByRef.get().then(async function(vendorNearByRefSnapshots) {
                ...
                /* The render loops below test each store synchronously, so
                 * the region ids have to be in hand before any of them run. */
        ...
        async function catHaveStores(categoryId) {
            ...
            /* A category whose only stores are in another region is empty to
             * this visitor, so it must not be offered. */
            for (var i = 0; i < snapshots.docs.length; i++) {
                if (storeMatchesRegion(snapshots.docs[i].data())) {
                    return true;
                }
            }
            return false;
    ...
    async function buildHTMLHomeCategoryProducts(category_id) {
        ...
        snapshots.docs.forEach((listval) => {
            ...
            if (!inValidVendors.includes(listval.id) && storeMatchesRegion(datas)) {
                alldata.push(datas);
            }
        });
