...
<div class="siddhi-home-page">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(async function() {
            ...
            /* The render loops below test each provider synchronously. */
            await discoveryRegionsReady;
            await ensureProviderRegions();
        ...
        async function buildAllServicesHTML(snapshots) {
            ...
            if (snapshots.docs.length > 0) {
                let promises = snapshots.docs.map(async (listval) => {
                    ...
                    if (serviceInzone && ( inValidProviders.length == 0 || !inValidProviders.includes(datas.author) ) && providerMatchesRegion(datas.author)) {
        ...
        function buildHTMLCouponList(couponListSnapshot) {
            ...
            couponListSnapshot.docs.forEach((listval) => {
                var datas = listval.data();
                datas.id = listval.id;
                if (!inValidProviders.includes(datas.providerId) && providerMatchesRegion(datas.providerId)) {
                    alldata.push(datas);
                }
            });
            if (alldata.length > 0) {
