    ...
    async function getProductList() {
        ...
        if (vendorsSnapshots.docs.length > 0) {
            vendorsSnapshots.docs.forEach((listval) => {
                if (!inValidVendors.includes(listval.id) && storeMatchesRegion(listval.data())) {
    ...
    async function getVendorList() {
        vendorsref.where('zoneId', '==', user_zone_id).get().then(async function(vendorsnapshot) {
            vendorsnapshot.docs.forEach((listval) => {
                ...
                if (!inValidVendors.includes(listval.id) && storeMatchesRegion(val)) {
                    vendordata.push(val);
                }
            });
        });
    ...
    async function getResults() {
        ...
        if (foodsearch != '') {
                ...
                for (i = 0; i < products.length; i++) {
                    ...
                    await database.collection('vendors').doc(vendorId).get().then(async function(snapshotss) {
                        if (snapshotss.exist) {
                            ...
                            if (!inValidVendors.includes(vendor_data.author) && storeMatchesRegion(vendor_data)) {
            ...
            vendordata.forEach((listval) => {
                ...
                var Ans = Name.indexOf(foodsearch.toLowerCase());
                if (Ans > -1) {
                    if (!inValidVendors.includes(data.author) && storeMatchesRegion(data)) {
                        vendors.push(data);
                    }
                }
            });
