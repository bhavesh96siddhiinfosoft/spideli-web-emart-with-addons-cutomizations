...
class ProductController extends Controller
{
     ...
     * The cart route.
     *
     * This returned view('checkout'), which does not exist - the view lives
     * at checkout/checkout.blade.php - so the route answered every request
     * with a 500. Nothing links to it, which is why it went unnoticed.
     *
     * It is not fixed by correcting the view name: that view needs $cart,
     * $errorMessage and $id - the Firestore uid of a signed-in user - and
     * this controller has no auth middleware, so it cannot supply them.
     * There is no separate cart view either; in this panel the checkout
     * page IS the cart. So the route forwards to the controller that builds
     * that page properly and applies auth.
     */
